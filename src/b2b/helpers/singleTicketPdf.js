...
const createSingleTicketPdf = async (activity, ticket) => {
  ...
  <div class="grid" style="grid-template-columns: repeat(3, 1fr); width: 100%; height: 150px; border-radius: 2xl; overflow: hidden; margin-top: 4px;">
    ${activity?.attraction?.images
      ?.slice(0, 3)
      ?.map((link, index) => {
        const firstImage = index === 0 ? 'border-top-left-radius: 2xl; border-bottom-left-radius: 2xl;' : '';
        const lastImage = index === activity.attraction.images.length - 1 ? 'border-top-right-radius: 2xl; border-bottom-right-radius: 2xl;' : '';
        return `
