  ...
  listAllVisaApplication: async (req, res) => {
    try {
      ...
      if (orderedBy == "b2c") {
        ...
        console.log(
          visaApplications[0]?.visaApplications[0],
          "visaApplications"
        );
    ...
    try {
      ...
      if (orderedBy == "b2c") {
        ...
        const visaApplication = await B2CVisaApplication.findOne(query, {
          ...
          .populate(
            "user referenceNumber createdAt totalAmount travellers.documents travellers.country"
          );
        ...
        const visaApplication = await VisaApplication.findOne(query, {
          ...
          .populate(
            "reseller referenceNumber createdAt totalAmount travellers.documents travellers.country"
          );
      ...
      if (orderedBy == "b2c") {
        ...
        if (!visaApplication.status == "payed") {
          return sendErrorResponse(
            res,
            400,
            "VisaApplication Amount Not Payed "
          );
        }
        ...
        res
          .status(200)
          .json({ status: true, message: "Visa Uploaded Succesfully " });
      } else {
        console.log(query, "query");
        const visaApplication = await VisaApplication.findOne(query)
        ...
        let reseller = await Reseller.findById(
          visaApplication.reseller
        ).populate("referredBy");
        ...
        if (reseller.role == "subAgent" && visaApplication.subAgentMarkup > 0) {
          const transaction = new B2BTransaction({
            ...
            amount:
              visaApplication.subAgentMarkup / visaApplication.noOfTravellers,
        ...
        if (visaApplication.resellerMarkup > 0) {
          const transaction = new B2BTransaction({
            ...
            paymentProcessor: "wallet",
            amount:
              visaApplication.resellerMarkup / visaApplication.noOfTravellers,
            order: visaApplication._id,
            orderItem: visaApplication.visaType,
          });
  ...
  cancelVisaApplicationStatus: async (req, res) => {
    try {
      ...
      if (orderedBy == "b2c") {
        ...
        res
          .status(200)
          .json({ status: true, message: "Visa rejected Succesfully " });
      } else {
        const visaApplication = await VisaApplication.findOne(query)
          .populate("reseller travellers.documents travellers.country")
          .populate({
